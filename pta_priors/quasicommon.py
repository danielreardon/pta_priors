"""
This module runs a calculation to distinguish pulsar noise with the same spectra from pulsar noise with different spectra across the pulsars. The degree of similarity is determined by parameter \sigma, where zero means that pulsar noise spectra are the same. Parameter \mu corresponds to the log-10 power spectral density amplitude of pulsar noise. For more information, please refer to arXiv: 2206.03766.

In practice, the calculation recycles the results from single-pulsar noise analyses.

Usage:
    python run_importance.py --result <result_file> --prfile <parameter_file> --target <target_file>

Arguments:
    --result: Path to the enterprise_warp parameter file for single-pulsar noise analysis (proposal likelihood).
    --target: Path to the enterprise_warp parameter file for the target likelihood (in practice, same as above).
    --prfile: Path to the parameter file HyperResult(results.BilbyWarpResult).

Citation:
    If you use this module, please cite the following publication:
    
    Goncharov, B. et al. 2022, "Consistency of the Parkes Pulsar Timing Array Signal with a Nanohertz Gravitational-wave Background," 
    ApJL, 932, L22. 
    DOI: https://doi.org/10.3847/2041-8213/ac76bb
...
def main():
  ...
  if opts.custom_models is not None and opts.custom_models_py is not None:
    ...
    custom = cmod.__dict__[opts.custom_models]
